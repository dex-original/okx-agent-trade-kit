...
```bash
curl -fsSL https://raw.githubusercontent.com/dex-original/okx-agent-trade-kit/master/scripts/install.sh | bash
```

**Windows (PowerShell):**

```powershell
irm https://raw.githubusercontent.com/dex-original/okx-agent-trade-kit/master/scripts/install.ps1 | iex
```
...
| `--modules <list>` | 逗号分隔的模块名，或 `all`。默认：`spot swap account`。`all` 包含所有模块（含赚币子模块）。 |
